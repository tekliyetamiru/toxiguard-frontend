...
export default function ManageBots() {
  ...
  const toggleBot = async (id, action) => {
    try {
      await api.post(`/api/user/bots/${id}/${action}`);
      await fetchData();
    } catch (err) { console.error(err); }
  };
  ...
  return (
    <ProtectedRoute>
      <Layout title="Telegram Bots">
        <div className="space-y-6">
          <div>
            <h1 className="text-3xl font-bold">💬 Telegram Bots</h1>
            <p className="text-gray-600 mt-1">Manage your Telegram moderation bots</p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              <div className="bg-gradient-to-r from-blue-500 to-cyan-500 -mx-6 -mt-6 px-6 py-4 rounded-t-xl mb-6">
                <h2 className="text-xl font-bold text-white">🤖 Add New Telegram Bot</h2>
              </div>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Bot Token</label>
                  <input type="text" name="token" value={form.token} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="123456:ABC-DEF..." required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Bot Username</label>
                  <input type="text" name="username" value={form.username} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="@my_bot" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">API Key</label>
                  <select name="api_key_id" value={form.api_key_id} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                ...
                <div>
                  ...
                  <textarea name="blocked_words" value={form.blocked_words} onChange={handleChange} rows="3" className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="badword1, badword2, ..." />
                </div>
                <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 text-white py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-cyan-600 disabled:opacity-50 transition shadow-md">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              ...
              {bots.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400">Add your first Telegram bot to start monitoring.</p>
                </div>
              ) : (
                    ...
                    <div key={bot.id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {bot.blocked_words && <p className="text-xs text-gray-400 mt-1">Blocked: {bot.blocked_words}</p>}
                        </div>
