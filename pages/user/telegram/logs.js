        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">View and analyze all messages processed from your Telegram bots</p>
            </div>
            <div className="flex gap-3">
              ...
              <button
                onClick={exportLogs}
                className="px-4 py-2 bg-blue-500 text-white rounded-lg font-medium hover:bg-blue-600 transition shadow-md"
              >
                📥 Export CSV
              </button>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-gray-500 text-lg">No messages logged yet for Telegram.</p>
                <p className="text-sm text-gray-400 mt-2">
                  {toxicOnly 
                    ? '🎉 No toxic messages detected! Your community is clean.' 
                    : 'Add a Telegram bot and start monitoring your groups.'}
                </p>
              </div>
            ) : (
                ...
                <table className="min-w-full">
                  <thead className="bg-gradient-to-r from-blue-50 to-cyan-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Message</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Language</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Owner</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Toxicity Level</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Status</th>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Categories</th>
                    </tr>
                  </thead>
                    ...
                    {messages.map((msg) => (
                      <tr key={msg.id} className={`transition ${msg.is_toxic ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-gray-50'}`}>
                        ...
                        <td className="px-6 py-4 text-sm">
                          <span className="px-2.5 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold uppercase">
