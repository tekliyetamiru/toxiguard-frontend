...
export default function DiscordAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      const [trendRes, botRes, msgsRes] = await Promise.all([
        ...
        api.get('/api/user/messages/discord?limit=500')
      ]);
  ...
  const trendChart = {
    ...
    datasets: [{ label: 'Messages', data: stats.dailyTrend.counts || [], borderColor: '#5865F2', backgroundColor: 'rgba(88,101,242,0.1)', tension: 0.4, fill: true }]
  };

  const pieChart = {
    labels: ['Toxic', 'Clean'],
    datasets: [{ data: [stats.toxicMessages, stats.nonToxic], backgroundColor: ['#ED4245', '#57F287'], borderWidth: 0 }]
  };

  const botBar = stats.botStats.length > 0 ? {
    labels: stats.botStats.map(b => b.username),
    datasets: [
      { label: 'Total', data: stats.botStats.map(b => b.message_count), backgroundColor: 'rgba(88,101,242,0.7)', borderRadius: 6 },
      { label: 'Toxic', data: stats.botStats.map(b => b.toxic_count), backgroundColor: 'rgba(237,66,69,0.7)', borderRadius: 6 },
    ]
  } : null;

  if (loading) return <ProtectedRoute><Layout title="Discord Analytics"><div className="flex justify-center py-20"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div></div></Layout></ProtectedRoute>;

  return (
    <ProtectedRoute>
      <Layout title="Discord Analytics">
        <div className="space-y-6">
          <div><h1 className="text-3xl font-bold">🎮 Discord Analytics</h1><p className="text-gray-600 mt-1">Detailed statistics for your Discord moderation</p></div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            {[
              { label: 'Total Messages', value: stats.totalMessages, color: 'border-indigo-500' },
              ...
              <motion.div key={i} whileHover={{ scale: 1.02 }} className={`bg-white rounded-xl shadow-md p-5 border-l-4 ${s.color}`}>
                <p className="text-gray-500 text-xs uppercase">{s.label}</p><p className={`text-2xl font-bold mt-1 ${s.text||''}`}>{s.value}</p>
              </motion.div>
            ))}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6"><h2 className="text-lg font-bold mb-4">🥧 Toxicity Distribution</h2><Pie data={pieChart} options={{ responsive: true, plugins: { legend: { position: 'bottom' } } }} />
              <div className="flex justify-center gap-4 mt-4"><span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full" style={{backgroundColor:'#ED4245'}}></span>Toxic: {stats.toxicMessages}</span><span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full" style={{backgroundColor:'#57F287'}}></span>Clean: {stats.nonToxic}</span></div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6"><h2 className="text-lg font-bold mb-4">🏆 Top Servers</h2>
              {stats.botStats.length === 0 ? <p className="text-gray-400 text-center py-8">No servers</p> : (
                <div className="space-y-2 max-h-80 overflow-auto">
                  {[...stats.botStats].sort((a,b)=>b.message_count-a.message_count).map((b,i)=>
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6">
            ...
            <p className="text-gray-700 mt-2">
              {stats.toxicRate > 10 ? `Your Discord toxicity rate is ${stats.toxicRate}%. Review your blocked words list.` : `Excellent! Low toxicity rate of ${stats.toxicRate}%. Your servers are well-moderated!`}
            </p>
            {stats.botStats.length > 0 && <p className="text-gray-600 mt-1">Most active: <strong>{[...stats.botStats].sort((a,b)=>b.message_count-a.message_count)[0]?.username}</strong></p>}
          </div>
        </div>
      </Layout>
    </ProtectedRoute>
